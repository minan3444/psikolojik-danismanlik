...
export default async function LegalPage({ params }) {
  ...
  if (!belge) {
    console.error(`HATA: Sanity'de "${decodedSlug}" slug'ına sahip belge bulunamadı!`);
    notFound();
  }

  return (
    <Box sx={{ pt: 16, pb: 10, bgcolor: 'background.default', minHeight: '100vh' }}>
      <Container maxWidth="md">
        <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} sx={{ mb: 4, color: 'text.secondary' }}>
          <Link href="/" style={{ textDecoration: 'none', color: 'inherit' }}>Anasayfa</Link>
          <Typography color="text.primary" sx={{ fontWeight: 500 }}>{belge.baslik}</Typography>
        </Breadcrumbs>

        <Typography variant="h3" sx={{ fontFamily: 'var(--font-playfair)', fontWeight: 700, mb: 2 }}>
          {belge.baslik}
        </Typography>
        <Box sx={{ width: 60, height: 4, bgcolor: 'primary.main', borderRadius: 2, mb: 6 }} />

        <Box sx={{ 
            lineHeight: 2, color: 'text.secondary', fontSize: '1.1rem',
            '& strong, & b': { color: 'text.primary', fontWeight: 700 },
            '& p': { mb: 3 }, '& ul': { mb: 3, pl: 3 }, '& li': { mb: 1.5 }
        }}>
