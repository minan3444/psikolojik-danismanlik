...
export default function VakaKarti({ vaka }) {
  ...
  return (
    ...
    <Link href={`/vaka-analizleri/${vaka.slug}`} style={{ textDecoration: 'none' }}>
      <MotionBox
      ...
      >
        ...
        >
          <Chip
            label={vaka.kategori}
            size="small"
            sx={{ position: 'absolute', top: 16, left: 16, bgcolor: 'primary.main', color: 'white', fontWeight: 600 }}
          />
        </Box>
        ...
        <Box sx={{ p: 3, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
          <Typography
            variant="h6"
            sx={{ fontFamily: 'var(--font-playfair)', fontWeight: 700, color: 'text.primary', mb: 1.5, fontSize: '1.1rem' }}
          >
            {vaka.baslik}
          </Typography>
          ...
          <Box sx={{ mt: 'auto' }}>
            <Button variant="text" color="primary" sx={{ p: 0, fontWeight: 700, fontSize: '0.8rem' }}>
              DETAYLARI OKU →
            </Button>
          </Box>
        </Box>
